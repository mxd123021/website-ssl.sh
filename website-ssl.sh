...
function init(){
    ...
    if [[ ! -f $openssl_cnf ]];then
        cp /root/website-ssl.sh/trunk/libs/openssl.cnf /etc/ssl/
    fi
}
...
function create_pem(){
    ...
    # 申请证书crt文件
    python3 /root/website-ssl.sh/trunk/libs/acme_tiny.py --account-key account.key --csr domain.csr --acme-dir $challenges_dir > signed.crt
